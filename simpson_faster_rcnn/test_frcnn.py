...
def get_models(C):
	...
	classifier = nn.classifier(feature_map_input, roi_input, C.num_rois, nb_classes=len(C.class_mapping),
	                           trainable=True)
...
def detect_predict(pic, C, model_rpn, model_classifier, model_classifier_only, class_mapping, class_to_color,
                   print_dets=False, export=False):
	"""
...
if __name__ == "__main__":
	...
	for idx, img_name in enumerate(sorted(os.listdir(options.test_path))):
		...
		st = time.time()
		img = detect_predict(img, C, model_rpn, model_classifier, model_classifier_only, class_mapping, class_to_color,
		                     True)
		print('Elapsed time = {}'.format(time.time() - st))
		cv2.imwrite('./results_test/{}_result_{}'.format(options.test_path.split('/')[-1], img_name), img)
